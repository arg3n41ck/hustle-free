...
const Edits = () => {
  ...
  const { current: validationSchema } = useRef(
    yup.object({
      ...
      nameOrganization: yup.string().nullable().required(tCommon('validation.required')),
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        const currentCountry = countries.find((country) => country.name === values.country),
          currentCity = currentCountry.cityCountry.find((country) => country.name === values.city),
          newValues = {
            ...values,
            dateBirthday:
      ...
      <Content>
        ...
        <div className='auth-wrapper__input'>
          <p className='auth-title__input'>
            {tCommon('form.fieldsNames.gender.label')} ({tCommon('form.fieldsNames.notNecessary')})
          </p>
        ...
        <div className='auth-wrapper__input'>
          <p className='auth-title__input'>
            {tCommon('form.fieldsNames.actualAddress')} ({tCommon('form.fieldsNames.notNecessary')})
          </p>
          <TextField
            sx={{
              width: '100%',
              '& .MuiOutlinedInput-root': {
                '& > fieldset': {
                  borderColor:
                    formik.touched.factAddress &&
                    Boolean(formik.errors.factAddress) &&
                    '#d32f2f !important',
                },
              },
            }}
