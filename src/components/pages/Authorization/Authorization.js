...
const Authorization = ({ onView }) => {
  ...
  const { current: validationSchema } = useRef(
    yup.object({
      ...
      password: yup
        .string()
        .test('', tCommon('validtion.required'), (value) => !!(value || ' ').replace(/\s/g, ''))
        .required(tCommon('validtion.required')),
    }),
  )
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        ...
        setErrorMessage(tAuth('toast.invalidCredentials'))
    ...
    >
      ...
      <div className='auth-container'>
        <div className='auth-wrapper'>
          <h3 className='auth-title'>{tAuth('login.loginH1')}</h3>
          <p className='auth-description'>{tAuth('login.loginP')} </p>
          ...
          <div className='auth-wrapper__input'>
            ...
            >
              <OutlinedInput
                ...
                placeholder={tAuth('common.enterPassword')}
              />
            </FormControl>
          </div>
          ...
          <div className='auth-additionally'>
            <Box
              ...
              component='p'
            >
              {tAuth('common.forgotPassword')}{' '}
              <span onClick={() => onView('recover')} className='auth-link'>
                {tAuth('common.restorePassword')}
              </span>
            </Box>
...
export const AuthButton = styled.button`
  width: 100%;
  background: ${({ active }) => {
    return active ? 'linear-gradient(90deg, #3F82E1 0%, #7A3FED 100%)' : '#F2F2F2'
  }};
  border-radius: 12px;
  height: 64px;
