...
function CommunitesAthletesPage() {
  ...
  const teaamsValue = teams?.length && teams?.find((type) => `${type.id}` === query.get('team'))

  const countriesValue =
    countries.length && countries.find((type) => `${type.id}` === query.get('country'))

  const gendersValue = gender.length && gender.find((type) => type.value === query.get('gender'))
  ...
  const handleTeamsTypesFilter = useCallback(
    (_, value) => {
      value ? query.set('team', value.id) : query.delete('team')
      routerPush(`/communities/athletes/?${query}`)
    },
    [query],
  )
